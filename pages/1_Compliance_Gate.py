...
st.title("🏗️ 1. Engineering Compliance Gate")
st.info("بوابة رفع ومطابقة مخططات إجازة البناء الإلكترونية الموحدة مع الكودات العراقية")

with st.container(border=True):
    st.subheader("⚙️ مدخلات الفحص والتدقيق البلدي والإنشائي")
    
    # خيارات اختيار المنطقة والتربة المربوطة بالدستور الرقمي
    zone = st.selectbox(
        "اختر المحافظة ونطاق المشروع الجغرافي:",
        ["شمال العراق (نينوى والمناطق الجبلية)", "جنوب العراق (البصرة والمحافظات الجنوبية)", "وسط العراق (بغداد ونواحيها)"]
    )
    ...
    building_height = st.number_input("أدخل الارتفاع الكلي للمنشأ المصمم (متر):", min_value=3, max_value=50, value=10)
    
    st.markdown("---")
    
    if st.button("🚨 تشغيل التدقيق الكودي والبلدي الفوري", type="primary"):
        # محاكاة بسيطة للتحقق العام للحفاظ على الهيكل القديم
        res = {"status": True, "message": "تم اجتياز الفحص المبدئي بنجاح."}
        ...
        if res["status"]:
            ...
            st.error("❌ فشل التحقق العام للمنصة.")
